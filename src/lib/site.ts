export const siteConfig = {
  name: 'Complete Trade Solutions',
  descriptor: 'Kitchen, bathroom, and trade renovation support',
  phone: '0421 601 954',
  phoneHref: 'tel:+61421601954',
  email: 'info@completetrade.au',
  location: 'Melbourne, VIC, Australia, Victoria',
  serviceArea: 'Melbourne metro and surrounding areas',
  facebookUrl:
    'https://www.facebook.com/profile.php?id=61577631743905&mibextid=wwXIfr',
  heroHeadline: [
    'Renovation and Remodeling',
    'Exterior Construction & Roof Restoration',
    'Painting Services',
    'Plumbing Services',
    'Electrical Services',
    'Cabinetry and Interior Fitouts',
    'Flooring',
  ],
  tagline:
    'Building Beyond Expectations, We specialise in End to End Commercial and Residental Improvements.',
}

export const serviceTickerItems = siteConfig.heroHeadline

export const services = [
  {
    title: 'Kitchen Renovations',
    text: 'A focused service area for kitchen updates, finishes, and coordinated trades.',
  },
  {
    title: 'Bathroom Renovations',
    text: 'A clear section for bathroom renovation enquiries and supplied project photos.',
  },
  {
    title: 'Coordinated Trades',
    text: 'A simple message for customers who want the renovation process managed clearly.',
  },
]

export const serviceTags = [
  'Kitchen updates',
  'Bathroom upgrades',
  'Seamless finishes',
  'Coordinated trades',
  'Quote enquiries',
]
